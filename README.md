...
b_server 클래스는 각 주차구역(클라이언트) 별로 연결 요청 처리 및 c_manager 클래스의 인스턴스를 생성 할 수 있음.<br>
c_manager 클래스에서는 각 클라이언트와 소켓으로 통신하여 데이터를 주고 받으며 DB 조회, 위반 차량 판단, DB에 주차 기록 저장/수정 및 JTable에 출력 등의 기능을 함.<br>
